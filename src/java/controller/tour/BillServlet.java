...
public class BillServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            // Tạo một đối tượng BillDAO
            BillDAO billDAO = new BillDAO(DatabaseConnector.getConnection());
            
            // Gọi phương thức getAllBills để lấy danh sách hóa đơn
            List<Bill> bills = billDAO.getAllBills();
            
            // Đặt danh sách hóa đơn vào thuộc tính của request để chuyển nó đến trang JSP
            request.setAttribute("bills", bills);
            
            // Chuyển hướng đến trang JSP để hiển thị danh sách hóa đơn
